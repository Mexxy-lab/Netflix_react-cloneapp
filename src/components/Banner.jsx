...
function Banner() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const request = await axios.get(
          "https://api.themoviedb.org/3/discover/tv?api_key=4a6fd5c43b94e7c2b26f73a1ddf3438a&with_networks=213"
        );
        const Random = Math.floor(Math.random() * 20);
        if (isMounted) UpdateMovies(request.data.results[Random]); // only update if mounted
      } catch (error) {
